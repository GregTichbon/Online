...
%>	
<HTML><HEAD><title>Incedo ConnectMe</title>
<style type="text/css">
<!--
.hideme {
	visibility: hidden;
			...
			with rs
				...
				for each frm in request.form
					if left(frm,12) = "organisation" then
						...
						.fields("firstname") = nullify(request.form("firstname" & myid))
...
<%
	...
	'response.write cnt
			...
			elseif duplicates <> "" then
				max = 200
				'sql = "SELECT id, organisation, Recipient, Surname, firstname, Title, Region, Status, PostalAddress, camefrom FROM ConnectMe WHERE (status is null or status = 'verified') and Surname like '" & duplicates & "%' and surname In (SELECT [Surname] FROM [ConnectMe] As Tmp where status is null or status = 'verified' GROUP BY [Surname] HAVING Count(*)>1) ORDER BY organisation, Surname, firstname"
				sql = "SELECT id, organisation, Recipient, Surname, firstname, Title, Region, Status, duplicateof, PostalAddress, camefrom FROM ConnectMe WHERE Surname >= '" & duplicates & "' and surname In (SELECT [Surname] FROM [ConnectMe] As Tmp GROUP BY [Surname] HAVING Count(*)>1) ORDER BY organisation, Surname, firstname, status"
	response.write sql
	'response.end
			else
				max = 200
				sql = "SELECT id as ID, Organisation as [Organisation], Recipient, Surname, firstname, Title, PostalAddress, Status, DuplicateOf, CameFrom from ConnectMe"
				usewhere = " where"
				useand = ""
				if searchname <> "" then
					max = 200
...
'response.end
			...
			do until rs.EOF
				...
				if 1=1 then
					...
					'Response.write "<td nowrap><input name=""firstname" & rs("id") & """ type=""text"" id=""firstname" & rs("id") & """ value=""" & rs("firstname") & """></td>"
					...
					response.write "<input name=""firstname" & rs("id") & """ type=""text"" id=""firstname" & rs("id") & """ value=""" & rs("firstname") & """><br>"
