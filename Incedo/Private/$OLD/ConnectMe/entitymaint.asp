<%
	...
	if id <> "new" then
		...
		if not rs.eof then
			...
			fname = "photos/120x120/" & id & ".jpg"
			if fso.FileExists (server.mappath(fname)) then 
				photo = "<img src=""" & fname & """><br>"
			end if
		else
...
<body>
<div align="center">
  <form action="entitymaint_process.asp" method="post" enctype="multipart/form-data" name="maint" OnSubmit="return checkform();">
...
%>
      <tr>
        ...
        <td><textarea name="notes" cols="50" rows=5 class="fields" id="notes"><%=notes%></textarea></td>
           ...
           <tr>
             <td width="150"><div align="right">Photo</div></td>
             <td><%=photo%><input name="$photo" type="file" id="$photo"></td>
           </tr>
      <tr>
