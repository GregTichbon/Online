...
<P>
<%
cmd.CommandText = "SELECT forumMESSAGES.ID, forumMESSAGES.Topic, forumMESSAGES.ReplyCount, forumMESSAGES.LastThreadPost, forumMESSAGES.DatePosted, [firstname] & ' ' & [lastname] AS AuthorName " & _
				  "FROM forumMESSAGES INNER JOIN People ON forumMESSAGES.AuthorID = People.ID " & _
...
<% 
if bNoRecords then
	response.write "<TR><TD COLSPAN=""5""><B>There are no messages available at the present time</B></TD></TR>"
else
	set di = server.createobject("DI.IIS")
	for lCtr = lPageStart to lPageEnd
		response.write "<TD><A HREF='showposts.asp?ID=" & rs("ID") & "'>"  & rs("Topic") & "</A></TD>"
		response.write " <TD>" & rs("AuthorName") & "</A></TD>"
