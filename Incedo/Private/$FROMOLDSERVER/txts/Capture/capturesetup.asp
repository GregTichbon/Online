...
<%
	...
	if id = "" then
		...
		if session("yfc-txtcapture") = "" then
			'get password
%>
<form action="capturesetup.asp" method="post" name="password" id="password">
...
<form action="capturesetup.asp" method="post" name="maint" id="maint">
    <input name="$id" type="hidden" id="$id" value="<%=id%>"> 
