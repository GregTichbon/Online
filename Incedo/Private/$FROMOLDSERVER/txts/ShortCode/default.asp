<%
	...
	'http://www.incedo.org.nz/private/txts/shortcode/?msg=Soxweather&mobile=0274266494&to=8244 Note the tag (INZ) does not come through
	msg = request.querystring("msg")
	msgnospaces = lcase(replace(msg," ",""))
	mobile = request.querystring("mobile")
	set di = server.createobject("DI.IIS")
	select case true
		case left(msgnospaces,4) = "test" and 1=2
			message = "Thanks - you've just spent 20c on a test - you must be made of money!"
			respondto(message)
		case left(msgnospaces,4) = "tohw"
			message = "Te Aroha Te Koari is an awesome youth worker!"
			respondto(message)
		case left(msgnospaces,6) = "summer" and 1=2
			message = "Thanks - you're in the draw!"
			respondto(message)
		case left(msgnospaces,8) = "mymentor" and 1=2
			message = "Thanks - we appreciate you taking the time to let us know who has had a significant input into your life."
			respondto(message)
		case left(msgnospaces,9) = "beamentor" and 1=2
		...
		case msgnospaces = "soxweather"
			Set objFSO = Server.CreateObject("Scripting.FileSystemObject")
			Set objTStream = objFSO.OpenTextFile("e:\homepage\incedo\private\txts\shortcode\soxweather.txt", 1)
...
'response.end
	...
	Set objFSO = Server.CreateObject("Scripting.FileSystemObject")
	Set objTStream = objFSO.OpenTextFile("e:\homepage\incedo\private\txts\shortcode\received.txt", 8, True)
...
function respondto(message)
	...
	elseif method = "post" then
		...
		Set objFSO = Server.CreateObject("Scripting.FileSystemObject")
		Set objTStream = objFSO.OpenTextFile("e:\homepage\incedo\private\txts\shortcode\debug.txt", 8, True)
