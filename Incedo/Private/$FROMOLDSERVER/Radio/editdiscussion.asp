...
function checkall(field) {
	for (i = 0; i < document.form1.elements.length; i++) {
		if(document.form1.elements[i].name.substring(0,3) == field) {
  ...
  <div class="outerbox">
      <h1>Radio Incedo</h1>
		...
		<tr><td>Name</td><td>Comment</td>
		  <td>Archive<br>
		    <input type="button" value="Check All" onClick="checkall('arc')">
		    <br>
            <input type="button" value="Uncheck All" onClick="uncheckall('arc')"></td>
		  <td>Delete<br>
		    <input type="button" value="Check All" onClick="checkall('del')">
		    <br>
            <input type="button" value="Uncheck All" onClick="uncheckall('del')"></td>
</td>
		</tr>
        <%
	rs.open sql,db
	do until rs.eof
		...
		response.write "<tr><td><input type=""text"" name=""name" & id & """ value=""" & usenames & """" & s & "></td><td><textarea name=""comment" & id & """" & s & ">" & rs("comment") & "</textarea></td><td><input name=""archived" & id & """ type=""checkbox"" value=""-1""" & checked & "></td><td><input name=""delete" & id & """ type=""checkbox"" value=""-1""></td></tr>"
		rs.movenext
	loop
	rs.close
%>
          <tr><td colspan="4"><input name="$submit" type="submit" id="$submit" value="Submit"></td></tr>
		  </table>
        </form>
</div>
</div>
