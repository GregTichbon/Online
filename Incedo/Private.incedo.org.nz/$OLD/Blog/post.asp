...
if editmessage = "new" then
	sSQL = "INSERT INTO blog (AUTHOR,title,blog) VALUES (" & request.form("memberid") & "," & sTopic & "," & sComments & ",'" & thedatetime & "','" & thedatetime & "')"
	conn.execute sSQL
	sSQL = "UPDATE blog SET THREADPARENT = ID WHERE THREADPARENT = 0"
	conn.execute sSQL
	message = "A new topic was added to the forum entitled: " & sTopic & " by " & name & "<br>Click <a href=""http://www.teorahou.org.nz/private/forum/default.asp"">Here</a> to go directly to the forums"
elseif editmessage = "1" then
	...
	if request.form("delete") = "Delete" then
		...
		sSQL = "UPDATE blog SET TOPIC = " & sTopic & ", COMMENTS = " & sComments & " WHERE ID = " & parentmessage
		conn.execute sSQL
	...
	sSQL = "INSERT INTO blog (AUTHORID,TOPIC,COMMENTS,THREADPARENT,PARENTMESSAGE,LASTTHREADPOST,DATEPOSTED) VALUES (" & request.form("memberid") & "," & sTopic & "," & sComments & "," & threadparent & "," & parentmessage & ",'" & thedatetime & "','" & thedatetime & "')"
	conn.execute sSQL
	sSQL = "UPDATE blog SET REPLYCOUNT = REPLYCOUNT + 1, LASTTHREADPOST = '" & thedatetime & "' WHERE ID = " & threadparent
	conn.execute sSQL
	message = "A new message was added under the topic entitled: " & sTopic & " by " & name & "<br>Click <a href=""http://www.teorahou.org.nz/private/forum/showposts.asp?id=" & threadparent & """>Here</a> to go directly to it"
end if
