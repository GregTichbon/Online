<HTML><HEAD><title>SQL</title></HEAD>
<BODY bgcolor="#99FFFF">
<p align="left"> 
  <%
		...
		if exec = "yes" then
			...
			if ucase(left(sql,6)) = "SELECT" then
				...
				if dump = "yes" then
					...
					Set objTStream = objFSO.OpenTextFile(Server.Mappath("\") & "\private\people\sqlresults.csv",2,true)
					objTStream.WriteLine (pagerec)
					objTStream.Close
					set objTStream = nothing
					set objFSO = nothing	
					Response.Write "<a href=""sqlresults.csv?id=" & now() & """>Download</a>"		
				else
