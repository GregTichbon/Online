<%
	if session("Incedo_MemberID")= "" then
		session("Incedo_Returnto") = "Polls/Email21July2009"
...
<%
		for f1=1 to 8
			sql = "select * from pollitem where pollid = " & pollid & " and responseid = " & session("Incedo_MemberID") & " and questionid = 'question" & f1 & "'"
	'response.write sql
	'response.end
			rs.open sql, db
			if not rs.eof then
