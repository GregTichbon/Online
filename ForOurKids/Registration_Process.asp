...
'response.end
		...
		if mode = "create" and request.form("$captcha") + 0 <> (request.form("$captchaverify") * 2 + 1) then
			response.write "Invalid submission"
			'response.write "<br>" & mode & "," & request.form("$captcha") & "," & request.form("$captchaverify") & "," & (request.form("$captchaverify") * 2 + 1)
		else 
...
if 1=1 and not local then	
		...
		myvar = "<br><br><table border=""1"">"
		For Each var in Request.ServerVariables
			myvar = myvar & "<tr><td>" & var & "</td><td>" & request.ServerVariables(var) & "&nbsp;</td></tr>"
		Next
		myvar = myvar & "<tr><td>" & "$captcha" & "</td><td>" & request.form("$captcha") & "&nbsp;</td></tr>"
		myvar = myvar & "<tr><td>" & "$captchaverify" & "</td><td>" & request.form("$captchaverify") & "&nbsp;</td></tr>"
		myvar = myvar & "</table>"
			...
			objMail.Body = acknowledgmentbody & myVar
